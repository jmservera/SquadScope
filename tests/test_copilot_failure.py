...
def test_classifies_copilot_token_failure_as_actionable_non_retryable() -> None:
    ...
    assert (
        copilot_failure.classify_log("HTTP 403 from Copilot").failure_class
        == "copilot_token_failure"
    )
    assert (
        copilot_failure.classify_log("HTTP 401 from Copilot").failure_class
        == "copilot_token_failure"
    )
    assert (
        copilot_failure.classify_log("HTTP 403 invalid token").failure_class
        == "copilot_token_failure"
    )
...
def test_classifies_context_timeout_and_transient_failures() -> None:
    assert (
        copilot_failure.classify_log("maximum context length exceeded").failure_class
        == "context_too_large"
    )
    assert copilot_failure.classify_log("request timed out").failure_class == "timeout"
    assert (
        copilot_failure.classify_log("HTTP 503 temporarily unavailable").failure_class
        == "transient_error"
    )
...
def test_create_or_update_token_issue_returns_consistent_url_for_existing_issue() -> None:
    ...
    responses = [
        mock.Mock(
            returncode=0,
            stdout='[{"number": 123, "title": "Renew GitHub Copilot token for weekly analysis workflow"}]',
            stderr="",
        ),
...
def test_create_or_update_token_issue_returns_consistent_url_for_created_issue() -> None:
    ...
    responses = [
        mock.Mock(returncode=0, stdout="[]", stderr=""),
        mock.Mock(
            returncode=0, stdout="https://github.com/jmservera/SquadScope/issues/124\n", stderr=""
        ),
    ]
