...
class WorkflowConfigTests(unittest.TestCase):
    ...
    def test_crawl_workflow_defines_analyze_job(self) -> None:
        ...
        self.assertGreaterEqual(run_analysis.count('"$WORKSPACE_GUARD" snapshot'), 2)
        self.assertGreaterEqual(run_analysis.count('"$WORKSPACE_GUARD" verify'), 2)
        self.assertIn('--root "$GITHUB_WORKSPACE"', run_analysis)
        self.assertIn("--available-tools=read,write", run_analysis)
        ...
        self.assertGreaterEqual(synthesis_run.count('"$WORKSPACE_GUARD" snapshot'), 2)
        self.assertGreaterEqual(synthesis_run.count('"$WORKSPACE_GUARD" verify'), 2)
        self.assertIn('--root "$GITHUB_WORKSPACE"', synthesis_run)
        self.assertIn("--available-tools=read,write", synthesis_run)
    ...
    def test_generate_hydration_preserves_committed_paths_absent_from_publish(self) -> None:
        ...
        restore_start = hydrate.index("git checkout HEAD --")
        self.assertGreater(
            restore_start,
            hydrate.index('git checkout origin/publish -- "$path"'),
        )
        restore_lines = hydrate[restore_start:].splitlines()[1:]
        restored_evidence = [
