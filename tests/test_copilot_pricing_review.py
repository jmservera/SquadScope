...
class CopilotPricingReviewTests(unittest.TestCase):
    ...
    def test_source_url_mismatch_requires_review(self) -> None:
        status = pricing_review.pricing_status(
            date(2026, 7, 1), source_url="https://example.invalid/pricing"
        )
        self.assertTrue(status["needs_review"])
    ...
    def test_source_headers_are_parsed_for_report_metadata(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=tests_root) as tmpdir:
            ...
            status = pricing_review.pricing_status(
                date(2026, 7, 1), source_headers=pricing_review.parse_source_headers(headers)
            )
    ...
    def test_main_writes_report_json_and_github_outputs(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=tests_root) as tmpdir:
            ...
            self.assertIn(
                "does not change pricing automatically", report_path.read_text(encoding="utf-8")
            )
...
class CopilotPricingReviewWorkflowTests(unittest.TestCase):
    def test_workflow_is_scheduled_and_opens_issue_without_changing_pricing(self) -> None:
        ...
        # issues:write is scoped to the job (least-privilege) rather than the workflow.
        ...
        metadata_step = next(
            (
                ...
                if step.get("name") == "Capture Copilot pricing source metadata"
            ),
            None,
        )
        ...
        issue_step = next(
            (
                step
                for step in job["steps"]
                if step.get("name") == "Create or update pricing review issue"
            ),
            None,
        )
