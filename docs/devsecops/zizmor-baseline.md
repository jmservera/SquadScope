...
* Hosted action: `zizmorcore/zizmor-action` v0.5.6 at commit
  `5f14fd08f7cf1cb1609c1e344975f152c7ee938d`

The hosted action receives `version: 1.27.0` and
`inputs: .github/workflows/`. It uses blocking console mode because the
action's Advanced Security mode intentionally suppresses finding exit codes.
The job therefore fails on regular-persona findings instead of relying on
stateful code-scanning merge protection.
...
For a credential-free offline check, use the same pinned version and scope:
