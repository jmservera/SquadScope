...
| FR-035 | Search Console | Connect and verify Google Search Console (and confirm GA4) for the production domain; submit sitemap. | G-002,G-004 | Search visitor | Must | GSC property verified, sitemap submitted; GA4 receiving data. | Complete: connection owner-confirmed 2026-08-02; dated GA4/GSC baseline captured 2026-08-08 (see `docs/growth/ga4-gsc-baseline-2026-07-29.md`). |
...
| FR-041 | Link-check gate | Validate internal links in CI; broken internal links fail the build gate. | G-005 | - | Should | CI link-check runs and fails on broken internal links. | Delivered: `.github/workflows/ci.yml` runs `scripts/check_internal_links.py` in the production-site job, plus test-level coverage (`tests/test_internal_link_checker.py`). |
...
|------|----------|-------|---------|--------|
| Q-01 | Quantify incremental generation cost/time for hubs, data, and repo pages | Leela | Design spike | In progress: the report-only experiment was dispatched on `main` 2026-08-08 ([run 31251984605](https://github.com/jmservera/SquadScope/actions/runs/31251984605)) but failed the workload guard (`topic_hubs count is 1; expected 5`). Root cause: the deploy/experiment hydration `rm -rf content/topics/ && git checkout publish -- content/topics/`, and `publish` only carries the legacy `ai-ml` hub `_index.md` (the 5 relaunch seed hubs live on `main`, not `publish`), so `discover_workload` counts 1 `_index.md` instead of 5. **No production impact** \u2014 verified 2026-08-08 that the live hubs (e.g., `/topics/ai-coding-agents/`) render with dataset highlights because topic term pages are generated from the weekly `topics:` taxonomy and highlights are template-driven, independent of the stripped `_index.md`. Harness fix (owner URL/Leela): publish the seed hub `_index.md` files to `publish`, or adjust the experiment's `topic_hubs` expectation to match the hydrated corpus |
...
| 1.6 | 2026-08-08 | SquadScope Squad | Captured the dated GA4/GSC baseline (FR-035), reconciled the FR-041 CI link-check note, dispatched the cost experiment (Q-01), and recorded the approved timing budget | Updated |
