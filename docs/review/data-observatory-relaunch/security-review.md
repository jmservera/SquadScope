...
| SEC-06 | GA4, GSC, and Podcaster secret behavior is not proven by repository inspection             | Medium        | URL and jmservera     | Repo-side check by URL 2026-08-04 (names/wiring only, no values recorded): `GA_MEASUREMENT_ID` secret exists and the GA4 script renders in the built site; `PODCASTER_API_KEY` exists at repo level and is correctly scoped to the `podcaster-real-generation` environment; `PODCASTER_ENDPOINT` is read as a repo **variable** (`vars.PODCASTER_ENDPOINT`) by every workflow, but a same-named repo **secret** also exists and is never referenced — orphaned, jmservera should confirm and remove it; a `GSC_SITE_VERIFICATION` secret is absent, so this site's own GSC meta-tag verification path is currently inactive, even though the owner-action-register's 2026-08-02 note records jmservera having verified the GSC property by some means — jmservera to confirm whether that verification used a different method (e.g. DNS) or whether this secret is still needed. External verification (GA4 event receipt, GSC processed-sitemap conclusion, Podcaster downstream confirmation) remains pending and out of repository-tooling scope |
