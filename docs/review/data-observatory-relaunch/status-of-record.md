...
| Real Podcaster downstream run (NFR-002 / R-04) | Pending | Manual exact-run workflow is bound to `podcaster-real-generation`; the environment now exists with a required reviewer and `main`-only deployment restriction (2026-08-04); endpoint/secret configuration, policy review, and one authorized dispatch remain pending |
