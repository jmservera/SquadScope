---
title: gin-gonic/gin repository trend history
description: 'Evergreen Claracle Observatory page for gin-gonic/gin: 6 weekly appearances, stars, velocity, topics, and related
  repos.'
date: '2026-08-03'
draft: false
layout: repo
generated_by: observatory_repo_pages
repo_owner: gin-gonic
repo_name: gin
repo_full_name: gin-gonic/gin
repo_url: https://github.com/gin-gonic/gin
repo_slug: gin-gonic-gin
repo_description: Gin is a high-performance HTTP web framework written in Go. It provides a Martini-like API but with significantly
  better performance—up to 40 times faster—thanks to httprouter. Gin is designed for building REST APIs, web applications,
  and microservices.
repo_language: Go
tags:
- framework
- gin
- go
- middleware
- performance
- router
- server
first_seen_week: 2026-W24
last_seen_week: 2026-W32
as_of_week: 2026-W32
source_paths:
- data/archive/recovered-W23-W29/2026-W24/2026-W24.json
- data/archive/recovered-W23-W29/2026-W26/2026-W26.json
- data/archive/recovered-W23-W29/2026-W27/2026-W27.json
- data/raw/2026-W29.json
- data/raw/2026-W30.json
- data/raw/2026-W32.json
distinct_weekly_issues: 6
recurrence_threshold:
  operator: '>'
  distinct_weekly_issues: 3
  minimum_weeks: 4
star_history:
- week: 2026-W24
  stars: 88635
  delta: null
- week: 2026-W26
  stars: 88728
  delta: 93
- week: 2026-W27
  stars: 88774
  delta: 46
- week: 2026-W29
  stars: 88921
  delta: 147
- week: 2026-W30
  stars: 88932
  delta: 11
- week: 2026-W32
  stars: 89012
  delta: 80
weekly_appearances:
- week: 2026-W24
  url: /weekly/2026/w24/
- week: 2026-W26
  url: /weekly/2026/w26/
- week: 2026-W27
  url: /weekly/2026/w27/
- week: 2026-W29
  url: /weekly/2026/w29/
- week: 2026-W30
  url: /weekly/2026/w30/
- week: 2026-W32
  url: /weekly/2026/w32/
tag_links:
- name: framework
  url: /tags/framework/
- name: gin
  url: /tags/gin/
- name: go
  url: /tags/go/
- name: middleware
  url: /tags/middleware/
- name: performance
  url: /tags/performance/
- name: router
  url: /tags/router/
- name: server
  url: /tags/server/
topic_links:
- name: Developer Tools
  slug: developer-tools
  url: /topics/developer-tools/
related_repos:
- full_name: avelino/awesome-go
  slug: avelino-awesome-go
  url: https://github.com/avelino/awesome-go
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - go
- full_name: coollabsio/coolify
  slug: coollabsio-coolify
  url: https://github.com/coollabsio/coolify
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - server
- full_name: fastapi/fastapi
  slug: fastapi-fastapi
  url: https://github.com/fastapi/fastapi
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - framework
- full_name: golang/go
  slug: golang-go
  url: https://github.com/golang/go
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - go
- full_name: grafana/grafana
  slug: grafana-grafana
  url: https://github.com/grafana/grafana
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - go
- full_name: junegunn/fzf
  slug: junegunn-fzf
  url: https://github.com/junegunn/fzf
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - go
- full_name: kubernetes/kubernetes
  slug: kubernetes-kubernetes
  url: https://github.com/kubernetes/kubernetes
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - go
- full_name: langchain-ai/langchain
  slug: langchain-ai-langchain
  url: https://github.com/langchain-ai/langchain
  score: 7
  shared_weeks:
  - 2026-W24
  - 2026-W26
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W32
  shared_topics:
  - framework
lifecycle:
  status: active
  as_of_week: 2026-W32
  retention_years: 3
  retained_until: null
  renamed_from: null
  prior_full_names: []
  renamed_to: null
  status_evidence: github_observation
  archived_at: null
  disabled_at: null
  deletion_confirmed_at: null
  note: ''
methodology_url: /methodology/
---

gin-gonic/gin has appeared in 6 Claracle weekly issues.

See the generated Observatory sections below for growth history, derived star velocity, weekly appearances, related repositories, and provenance.
