---
title: fffaraz/awesome-cpp repository trend history
description: 'Evergreen Claracle Observatory page for fffaraz/awesome-cpp: 5 weekly appearances, stars, velocity, topics,
  and related repos.'
date: '2026-07-29'
draft: false
layout: repo
generated_by: observatory_repo_pages
repo_owner: fffaraz
repo_name: awesome-cpp
repo_full_name: fffaraz/awesome-cpp
repo_url: https://github.com/fffaraz/awesome-cpp
repo_slug: fffaraz-awesome-cpp
repo_description: A curated list of awesome C++ (or C) frameworks, libraries, resources, and shiny things. Inspired by awesome-...
  stuff.
repo_language: ''
topics:
- awesome
- awesome-list
- c
- c-plus-plus
- cpp
- cpp-library
- cppcon
- libraries
- list
- lists
- programming-tutorial
- resources
first_seen_week: 2026-W22
last_seen_week: 2026-W30
as_of_week: 2026-W30
source_paths:
- data/archive/recovered-W23-W29/2026-W23/2026-W23.json
- data/archive/recovered-W23-W29/2026-W28/2026-W28.json
- data/raw/2026-W22.json
- data/raw/2026-W29.json
- data/raw/2026-W30.json
distinct_weekly_issues: 5
recurrence_threshold:
  operator: '>'
  distinct_weekly_issues: 3
  minimum_weeks: 4
star_history:
- week: 2026-W22
  stars: 71435
  delta: null
- week: 2026-W23
  stars: 71554
  delta: 119
- week: 2026-W28
  stars: 72105
  delta: 551
- week: 2026-W29
  stars: 72284
  delta: 179
- week: 2026-W30
  stars: 72333
  delta: 49
weekly_appearances:
- week: 2026-W22
  url: /weekly/2026/w22/
- week: 2026-W23
  url: /weekly/2026/w23/
- week: 2026-W28
  url: /weekly/2026/w28/
- week: 2026-W29
  url: /weekly/2026/w29/
- week: 2026-W30
  url: /weekly/2026/w30/
topic_links:
- name: awesome
  url: /topics/awesome/
- name: awesome-list
  url: /topics/awesome-list/
- name: c
  url: /topics/c/
- name: c-plus-plus
  url: /topics/c-plus-plus/
- name: cpp
  url: /topics/cpp/
- name: cpp-library
  url: /topics/cpp-library/
- name: cppcon
  url: /topics/cppcon/
- name: libraries
  url: /topics/libraries/
- name: list
  url: /topics/list/
- name: lists
  url: /topics/lists/
- name: programming-tutorial
  url: /topics/programming-tutorial/
- name: resources
  url: /topics/resources/
related_repos:
- full_name: jaywcjlove/awesome-mac
  slug: jaywcjlove-awesome-mac
  url: https://github.com/jaywcjlove/awesome-mac
  score: 8
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - awesome
  - awesome-list
  - list
- full_name: awesome-selfhosted/awesome-selfhosted
  slug: awesome-selfhosted-awesome-selfhosted
  url: https://github.com/awesome-selfhosted/awesome-selfhosted
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - awesome
  - awesome-list
- full_name: avelino/awesome-go
  slug: avelino-awesome-go
  url: https://github.com/avelino/awesome-go
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - awesome
  - awesome-list
- full_name: obsproject/obs-studio
  slug: obsproject-obs-studio
  url: https://github.com/obsproject/obs-studio
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - c
  - c-plus-plus
- full_name: public-apis/public-apis
  slug: public-apis-public-apis
  url: https://github.com/public-apis/public-apis
  score: 6
  shared_weeks:
  - 2026-W23
  - 2026-W28
  - 2026-W29
  shared_topics:
  - list
  - lists
  - resources
- full_name: vinta/awesome-python
  slug: vinta-awesome-python
  url: https://github.com/vinta/awesome-python
  score: 6
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - awesome
- full_name: f/prompts.chat
  slug: f-prompts-chat
  url: https://github.com/f/prompts.chat
  score: 6
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - awesome-list
- full_name: ripienaar/free-for-dev
  slug: ripienaar-free-for-dev
  url: https://github.com/ripienaar/free-for-dev
  score: 6
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W28
  - 2026-W29
  - 2026-W30
  shared_topics:
  - awesome-list
lifecycle:
  status: active
  as_of_week: 2026-W30
  retention_years: 3
  retained_until: null
  renamed_from: null
  renamed_to: null
  note: ''
methodology_url: /methodology/
---

fffaraz/awesome-cpp has appeared in 5 Claracle weekly issues.

See the generated Observatory sections below for growth history, derived star velocity, weekly appearances, related repositories, and provenance.
