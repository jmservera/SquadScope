---
title: nexu-io/open-design repository trend history
description: 'Evergreen Claracle Observatory page for nexu-io/open-design: 11 weekly appearances, stars, velocity, topics,
  and related repos.'
date: '2026-08-03'
draft: false
layout: repo
generated_by: observatory_repo_pages
repo_owner: nexu-io
repo_name: open-design
repo_full_name: nexu-io/open-design
repo_url: https://github.com/nexu-io/open-design
repo_slug: nexu-io-open-design
repo_description: '🎨 The open-source Claude Design alternative. 🖥️ Local-first desktop app. 🖼️ Your coding agent becomes the
  design engine: prototypes, landing pages, dashboards, slides, images & video — real files, HTML/PDF/PPTX/MP4 export. 🤖 Claude
  Code / Codex / Cursor / Gemini / OpenCode / Qwen & 20+ CLIs via BYOK.'
repo_language: TypeScript
tags:
- agent-skills
- ai-agents
- ai-design
- byok
- claude-code-for-design
- claude-design
- coding-agents
- design-systems
- design-tools
- desktop-app
- figma-alternative
- generative-ai
first_seen_week: 2026-W22
last_seen_week: 2026-W32
as_of_week: 2026-W32
source_paths:
- data/archive/recovered-W23-W29/2026-W23/2026-W23.json
- data/archive/recovered-W23-W29/2026-W24/2026-W24.json
- data/archive/recovered-W23-W29/2026-W25/2026-W25.json
- data/archive/recovered-W23-W29/2026-W26/2026-W26.json
- data/archive/recovered-W23-W29/2026-W27/2026-W27.json
- data/archive/recovered-W23-W29/2026-W28/2026-W28.json
- data/raw/2026-W22.json
- data/raw/2026-W29.json
- data/raw/2026-W30.json
- data/raw/2026-W31.json
- data/raw/2026-W32.json
distinct_weekly_issues: 11
recurrence_threshold:
  operator: '>'
  distinct_weekly_issues: 3
  minimum_weeks: 4
star_history:
- week: 2026-W22
  stars: 51653
  delta: null
- week: 2026-W23
  stars: 56789
  delta: 5136
- week: 2026-W24
  stars: 61424
  delta: 4635
- week: 2026-W25
  stars: 65722
  delta: 4298
- week: 2026-W26
  stars: 68741
  delta: 3019
- week: 2026-W27
  stars: 72530
  delta: 3789
- week: 2026-W28
  stars: 75272
  delta: 2742
- week: 2026-W29
  stars: 79310
  delta: 4038
- week: 2026-W30
  stars: 79766
  delta: 456
- week: 2026-W31
  stars: 81772
  delta: 2006
- week: 2026-W32
  stars: 83243
  delta: 1471
weekly_appearances:
- week: 2026-W22
  url: /weekly/2026/w22/
- week: 2026-W23
  url: /weekly/2026/w23/
- week: 2026-W24
  url: /weekly/2026/w24/
- week: 2026-W25
  url: /weekly/2026/w25/
- week: 2026-W26
  url: /weekly/2026/w26/
- week: 2026-W27
  url: /weekly/2026/w27/
- week: 2026-W28
  url: /weekly/2026/w28/
- week: 2026-W29
  url: /weekly/2026/w29/
- week: 2026-W30
  url: /weekly/2026/w30/
- week: 2026-W31
  url: /weekly/2026/w31/
- week: 2026-W32
  url: /weekly/2026/w32/
tag_links:
- name: agent-skills
  url: /tags/agent-skills/
- name: ai-agents
  url: /tags/ai-agents/
- name: ai-design
  url: /tags/ai-design/
- name: byok
  url: /tags/byok/
- name: claude-code-for-design
  url: /tags/claude-code-for-design/
- name: claude-design
  url: /tags/claude-design/
- name: coding-agents
  url: /tags/coding-agents/
- name: design-systems
  url: /tags/design-systems/
- name: design-tools
  url: /tags/design-tools/
- name: desktop-app
  url: /tags/desktop-app/
- name: figma-alternative
  url: /tags/figma-alternative/
- name: generative-ai
  url: /tags/generative-ai/
topic_links:
- name: AI Coding Agents
  slug: ai-coding-agents
  url: /topics/ai-coding-agents/
related_repos:
- full_name: Mintplex-Labs/anything-llm
  slug: mintplex-labs-anything-llm
  url: https://github.com/Mintplex-Labs/anything-llm
  score: 15
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - ai-agents
  - hermes-agent
  - local-first
  - no-code
- full_name: langgenius/dify
  slug: langgenius-dify
  url: https://github.com/langgenius/dify
  score: 14
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - claude
  - nextjs
  - no-code
- full_name: NousResearch/hermes-agent
  slug: nousresearch-hermes-agent
  url: https://github.com/NousResearch/hermes-agent
  score: 14
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - ai-agents
  - claude
  - hermes-agent
- full_name: affaan-m/ECC
  slug: affaan-m-ecc
  url: https://github.com/affaan-m/ECC
  score: 13
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - ai-agents
  - claude
- full_name: code-yeongyu/oh-my-openagent
  slug: code-yeongyu-oh-my-openagent
  url: https://github.com/code-yeongyu/oh-my-openagent
  score: 13
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - ai-agents
  - claude
- full_name: f/prompts.chat
  slug: f-prompts-chat
  url: https://github.com/f/prompts.chat
  score: 13
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - claude
  - nextjs
- full_name: farion1231/cc-switch
  slug: farion1231-cc-switch
  url: https://github.com/farion1231/cc-switch
  score: 13
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - desktop-app
  - hermes-agent
- full_name: langchain-ai/langchain
  slug: langchain-ai-langchain
  url: https://github.com/langchain-ai/langchain
  score: 13
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - ai-agents
  - generative-ai
lifecycle:
  status: active
  as_of_week: 2026-W32
  retention_years: 3
  retained_until: null
  renamed_from: null
  prior_full_names: []
  renamed_to: null
  status_evidence: github_observation
  archived_at: null
  disabled_at: null
  deletion_confirmed_at: null
  note: ''
methodology_url: /methodology/
---

nexu-io/open-design has appeared in 11 Claracle weekly issues.

See the generated Observatory sections below for growth history, derived star velocity, weekly appearances, related repositories, and provenance.
