---
title: jgraph/drawio-desktop repository trend history
description: 'Evergreen Claracle Observatory page for jgraph/drawio-desktop: 11 weekly appearances, stars, velocity, topics,
  and related repos.'
date: '2026-07-29'
draft: false
layout: repo
generated_by: observatory_repo_pages
repo_owner: jgraph
repo_name: drawio-desktop
repo_full_name: jgraph/drawio-desktop
repo_url: https://github.com/jgraph/drawio-desktop
repo_slug: jgraph-drawio-desktop
repo_description: Official electron build of draw.io
repo_language: JavaScript
topics:
- diagram-editor
- electron-app
- graphics
- javascript-applications
first_seen_week: 2026-W21
last_seen_week: 2026-W31
as_of_week: 2026-W31
source_paths:
- data/archive/recovered-W23-W29/2026-W23/2026-W23.json
- data/archive/recovered-W23-W29/2026-W24/2026-W24.json
- data/archive/recovered-W23-W29/2026-W25/2026-W25.json
- data/archive/recovered-W23-W29/2026-W26/2026-W26.json
- data/archive/recovered-W23-W29/2026-W27/2026-W27.json
- data/archive/recovered-W23-W29/2026-W28/2026-W28.json
- data/raw/2026-W21.json
- data/raw/2026-W22.json
- data/raw/2026-W29.json
- data/raw/2026-W30.json
- data/raw/2026-W31.json
distinct_weekly_issues: 11
recurrence_threshold:
  operator: '>'
  distinct_weekly_issues: 3
  minimum_weeks: 4
star_history:
- week: 2026-W21
  stars: 61150
  delta: null
- week: 2026-W22
  stars: 61279
  delta: 129
- week: 2026-W23
  stars: 61404
  delta: 125
- week: 2026-W24
  stars: 61496
  delta: 92
- week: 2026-W25
  stars: 61622
  delta: 126
- week: 2026-W26
  stars: 61735
  delta: 113
- week: 2026-W27
  stars: 61855
  delta: 120
- week: 2026-W28
  stars: 61976
  delta: 121
- week: 2026-W29
  stars: 62155
  delta: 179
- week: 2026-W30
  stars: 62183
  delta: 28
- week: 2026-W31
  stars: 62312
  delta: 129
weekly_appearances:
- week: 2026-W21
  url: /weekly/2026/w21/
- week: 2026-W22
  url: /weekly/2026/w22/
- week: 2026-W23
  url: /weekly/2026/w23/
- week: 2026-W24
  url: /weekly/2026/w24/
- week: 2026-W25
  url: /weekly/2026/w25/
- week: 2026-W26
  url: /weekly/2026/w26/
- week: 2026-W27
  url: /weekly/2026/w27/
- week: 2026-W28
  url: /weekly/2026/w28/
- week: 2026-W29
  url: /weekly/2026/w29/
- week: 2026-W30
  url: /weekly/2026/w30/
- week: 2026-W31
  url: /weekly/2026/w31/
topic_links:
- name: diagram-editor
  url: /topics/diagram-editor/
- name: electron-app
  url: /topics/electron-app/
- name: graphics
  url: /topics/graphics/
- name: javascript-applications
  url: /topics/javascript-applications/
related_repos:
- full_name: freeCodeCamp/freeCodeCamp
  slug: freecodecamp-freecodecamp
  url: https://github.com/freeCodeCamp/freeCodeCamp
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: openclaw/openclaw
  slug: openclaw-openclaw
  url: https://github.com/openclaw/openclaw
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: awesome-selfhosted/awesome-selfhosted
  slug: awesome-selfhosted-awesome-selfhosted
  url: https://github.com/awesome-selfhosted/awesome-selfhosted
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: torvalds/linux
  slug: torvalds-linux
  url: https://github.com/torvalds/linux
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: obra/superpowers
  slug: obra-superpowers
  url: https://github.com/obra/superpowers
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: tensorflow/tensorflow
  slug: tensorflow-tensorflow
  url: https://github.com/tensorflow/tensorflow
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: n8n-io/n8n
  slug: n8n-io-n8n
  url: https://github.com/n8n-io/n8n
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
- full_name: ohmyzsh/ohmyzsh
  slug: ohmyzsh-ohmyzsh
  url: https://github.com/ohmyzsh/ohmyzsh
  score: 11
  shared_weeks:
  - 2026-W21
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  shared_topics: []
lifecycle:
  status: active
  as_of_week: 2026-W31
  retention_years: 3
  retained_until: null
  renamed_from: null
  renamed_to: null
  note: ''
methodology_url: /methodology/
---

jgraph/drawio-desktop has appeared in 11 Claracle weekly issues.

See the generated Observatory sections below for growth history, derived star velocity, weekly appearances, related repositories, and provenance.
