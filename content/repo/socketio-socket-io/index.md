---
title: socketio/socket.io repository trend history
description: 'Evergreen Claracle Observatory page for socketio/socket.io: 9 weekly appearances, stars, velocity, topics, and
  related repos.'
date: '2026-07-29'
draft: false
layout: repo
generated_by: observatory_repo_pages
repo_owner: socketio
repo_name: socket.io
repo_full_name: socketio/socket.io
repo_url: https://github.com/socketio/socket.io
repo_slug: socketio-socket-io
repo_description: Bidirectional and low-latency communication for every platform
repo_language: TypeScript
topics:
- javascript
- nodejs
- socket-io
- websocket
first_seen_week: 2026-W22
last_seen_week: 2026-W31
as_of_week: 2026-W31
source_paths:
- data/archive/recovered-W23-W29/2026-W23/2026-W23.json
- data/archive/recovered-W23-W29/2026-W24/2026-W24.json
- data/archive/recovered-W23-W29/2026-W25/2026-W25.json
- data/archive/recovered-W23-W29/2026-W26/2026-W26.json
- data/archive/recovered-W23-W29/2026-W27/2026-W27.json
- data/raw/2026-W22.json
- data/raw/2026-W29.json
- data/raw/2026-W30.json
- data/raw/2026-W31.json
distinct_weekly_issues: 9
recurrence_threshold:
  operator: '>'
  distinct_weekly_issues: 3
  minimum_weeks: 4
star_history:
- week: 2026-W22
  stars: 63099
  delta: null
- week: 2026-W23
  stars: 63106
  delta: 7
- week: 2026-W24
  stars: 63132
  delta: 26
- week: 2026-W25
  stars: 63171
  delta: 39
- week: 2026-W26
  stars: 63186
  delta: 15
- week: 2026-W27
  stars: 63201
  delta: 15
- week: 2026-W29
  stars: 63157
  delta: -44
- week: 2026-W30
  stars: 63163
  delta: 6
- week: 2026-W31
  stars: 63171
  delta: 8
weekly_appearances:
- week: 2026-W22
  url: /weekly/2026/w22/
- week: 2026-W23
  url: /weekly/2026/w23/
- week: 2026-W24
  url: /weekly/2026/w24/
- week: 2026-W25
  url: /weekly/2026/w25/
- week: 2026-W26
  url: /weekly/2026/w26/
- week: 2026-W27
  url: /weekly/2026/w27/
- week: 2026-W29
  url: /weekly/2026/w29/
- week: 2026-W30
  url: /weekly/2026/w30/
- week: 2026-W31
  url: /weekly/2026/w31/
topic_links:
- name: javascript
  url: /topics/javascript/
- name: nodejs
  url: /topics/nodejs/
- name: socket-io
  url: /topics/socket-io/
- name: websocket
  url: /topics/websocket/
related_repos:
- full_name: freeCodeCamp/freeCodeCamp
  slug: freecodecamp-freecodecamp
  url: https://github.com/freeCodeCamp/freeCodeCamp
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
- full_name: electron/electron
  slug: electron-electron
  url: https://github.com/electron/electron
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
- full_name: nodejs/node
  slug: nodejs-node
  url: https://github.com/nodejs/node
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
- full_name: axios/axios
  slug: axios-axios
  url: https://github.com/axios/axios
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
- full_name: immich-app/immich
  slug: immich-app-immich
  url: https://github.com/immich-app/immich
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
- full_name: oven-sh/bun
  slug: oven-sh-bun
  url: https://github.com/oven-sh/bun
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
- full_name: louislam/uptime-kuma
  slug: louislam-uptime-kuma
  url: https://github.com/louislam/uptime-kuma
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - socket-io
  - websocket
- full_name: strapi/strapi
  slug: strapi-strapi
  url: https://github.com/strapi/strapi
  score: 11
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W24
  - 2026-W25
  - 2026-W26
  - 2026-W27
  shared_topics:
  - javascript
  - nodejs
lifecycle:
  status: active
  as_of_week: 2026-W31
  retention_years: 3
  retained_until: null
  renamed_from: null
  renamed_to: null
  note: ''
methodology_url: /methodology/
---

socketio/socket.io has appeared in 9 Claracle weekly issues.

See the generated Observatory sections below for growth history, derived star velocity, weekly appearances, related repositories, and provenance.
