---
title: microsoft/markitdown repository trend history
description: 'Evergreen Claracle Observatory page for microsoft/markitdown: 6 weekly appearances, stars, velocity, topics,
  and related repos.'
date: '2026-07-29'
draft: false
layout: repo
generated_by: observatory_repo_pages
repo_owner: microsoft
repo_name: markitdown
repo_full_name: microsoft/markitdown
repo_url: https://github.com/microsoft/markitdown
repo_slug: microsoft-markitdown
repo_description: Python tool for converting files and office documents to Markdown.
repo_language: Python
topics:
- autogen
- autogen-extension
- langchain
- markdown
- microsoft-office
- openai
- pdf
first_seen_week: 2026-W22
last_seen_week: 2026-W31
as_of_week: 2026-W31
source_paths:
- data/archive/recovered-W23-W29/2026-W23/2026-W23.json
- data/archive/recovered-W23-W29/2026-W27/2026-W27.json
- data/raw/2026-W22.json
- data/raw/2026-W29.json
- data/raw/2026-W30.json
- data/raw/2026-W31.json
distinct_weekly_issues: 6
recurrence_threshold:
  operator: '>'
  distinct_weekly_issues: 3
  minimum_weeks: 4
star_history:
- week: 2026-W22
  stars: 125043
  delta: null
- week: 2026-W23
  stars: 136691
  delta: 11648
- week: 2026-W27
  stars: 160755
  delta: 24064
- week: 2026-W29
  stars: 166870
  delta: 6115
- week: 2026-W30
  stars: 167400
  delta: 530
- week: 2026-W31
  stars: 169250
  delta: 1850
weekly_appearances:
- week: 2026-W22
  url: /weekly/2026/w22/
- week: 2026-W23
  url: /weekly/2026/w23/
- week: 2026-W27
  url: /weekly/2026/w27/
- week: 2026-W29
  url: /weekly/2026/w29/
- week: 2026-W30
  url: /weekly/2026/w30/
- week: 2026-W31
  url: /weekly/2026/w31/
topic_links:
- name: autogen
  url: /topics/autogen/
- name: autogen-extension
  url: /topics/autogen-extension/
- name: langchain
  url: /topics/langchain/
- name: markdown
  url: /topics/markdown/
- name: microsoft-office
  url: /topics/microsoft-office/
- name: openai
  url: /topics/openai/
- name: pdf
  url: /topics/pdf/
related_repos:
- full_name: langchain-ai/langchain
  slug: langchain-ai-langchain
  url: https://github.com/langchain-ai/langchain
  score: 8
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - langchain
  - openai
- full_name: docling-project/docling
  slug: docling-project-docling
  url: https://github.com/docling-project/docling
  score: 8
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - markdown
  - pdf
- full_name: Significant-Gravitas/AutoGPT
  slug: significant-gravitas-autogpt
  url: https://github.com/Significant-Gravitas/AutoGPT
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - openai
- full_name: f/prompts.chat
  slug: f-prompts-chat
  url: https://github.com/f/prompts.chat
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - openai
- full_name: NousResearch/hermes-agent
  slug: nousresearch-hermes-agent
  url: https://github.com/NousResearch/hermes-agent
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - openai
- full_name: langgenius/dify
  slug: langgenius-dify
  url: https://github.com/langgenius/dify
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - openai
- full_name: open-webui/open-webui
  slug: open-webui-open-webui
  url: https://github.com/open-webui/open-webui
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - openai
- full_name: firecrawl/firecrawl
  slug: firecrawl-firecrawl
  url: https://github.com/firecrawl/firecrawl
  score: 7
  shared_weeks:
  - 2026-W22
  - 2026-W23
  - 2026-W27
  - 2026-W29
  - 2026-W30
  - 2026-W31
  shared_topics:
  - markdown
lifecycle:
  status: active
  as_of_week: 2026-W31
  retention_years: 3
  retained_until: null
  renamed_from: null
  renamed_to: null
  note: ''
methodology_url: /methodology/
---

microsoft/markitdown has appeared in 6 Claracle weekly issues.

See the generated Observatory sections below for growth history, derived star velocity, weekly appearances, related repositories, and provenance.
